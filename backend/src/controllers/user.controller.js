...
export async function getRecommendedUsers(req, res) {
  try {
    ...
    const recommendedUsers = await User.find({
      $and: [
        { _id: { $ne: currentUserId } }, //exclude current user ie myself
        { _id: { $nin: currentUser.friends } }, // exclude current user's friends from recommended friends list
        { isOnboarded: true },
      ],
    });
...
export async function getMyFriends(req, res) {
  try {
    const user = await User.findById(req.user.id)
      ...
      .populate(
        "friends",
        "fullName profilePic nativeLangauge learningLanguage"
      );
...
export async function sendFriendRequest(req, res) {
  try {
    ...
    // prevent request sending to myself
    if (myId == recipientId) {
      return res
        .status(400)
...
export async function acceptFriendRequest(req, res) {
  try {
    ...
    // $addToSet is a method which adds elemetns to an array if they do not already exists
...
export async function rejectFriendRequest(req, res) {
  try {
    ...
    // Delete the friend request document from DB
